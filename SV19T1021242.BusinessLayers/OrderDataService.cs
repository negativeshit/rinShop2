...
namespace SV19T1021242.BusinessLayers
{
    public class OrderDataService
    {
        ...
        /// <returns></returns>
        public static bool SaveOrderProvinceAndAddress(int orderID, string deliveryProvince, string deliveryAddress)
        {
            Order? data = orderDB.Get(orderID);
            if (data == null)
                return false;
            if (data.Status == Constants.ORDER_INIT)
            {
                return orderDB.SaveProvinceAndAddress(orderID, deliveryProvince, deliveryAddress);
            }
            return false;
        }
        /// <summary>
